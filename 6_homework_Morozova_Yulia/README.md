...
Теперь согласно заданию, я изменю пару байт в таблице, для этого в редакторе я внесу изменения в файл, который определила в предыдущем пункте, это файл таблицы ``test`` : ``/var/lib/postgresql/15/test/base/5/16388``, далее включу кластер и сделаю селект из этой же таблицы ``test``(команды аналогичные пунктам выше) и так как у меня включен механизм контрольных сумм ,то я ожидаемо получаю ошибку при селекте из таблицы ``test``:

  ![6_7](https://github.com/Y-M-Morozova/Postgre-DBA-2023-11_OTUS_Morozova_Yulia/assets/153178571/50e343f3-4a08-43b2-b32e-15adde04c3d0)

Чтобы проигнорировать ошибку и продолжить работу в отсутствии резервной копии, я использую параметр ``ignore_checksum_failure``, который позволяет попробовать прочитать таблицу с риском получить искаженные данные, для этого выполню команду(включаем опцию игнорирования ошибок ``checksum``):

```sql
  SET ignore_checksum_failure = on;
```

Теперь у меня селект из таблицы становится возможен, но с предупреждением об ошибках:

  ![6_8](https://github.com/Y-M-Morozova/Postgre-DBA-2023-11_OTUS_Morozova_Yulia/assets/153178571/d6c6094a-38f7-4a5c-954e-6aad28a8684f)
