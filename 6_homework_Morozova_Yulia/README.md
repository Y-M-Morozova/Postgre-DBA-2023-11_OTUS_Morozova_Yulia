...
Теперь согласно заданию, я изменю пару байт в таблице, для этого в редакторе я внесу изменения в файл, который определила в предыдущем пункте, это файл таблицы ``test`` : ``/var/lib/postgresql/15/test/base/5/16388``, далее включу кластер и сделаю селект из этой же таблицы ``test``:
